...
class Slist
 ...
 public:
  ...
  {
    EmplaceFrontNode(CreateNode(std::forward<Args>(args)...));
  }
  ...
  {
    return SearchIf([&value](value_type const &val) {
      return val == value;
    });
  }
  ...
  {
    return ExtractNodeIf([&value](value_type const &val) {
      return value == val;
    });
  }
  ...
  void PopFront() { DropNode(ExtractFront()); }
  ...
  void EraseAfter(const_iterator pos) { EraseAfterNode(pos.node_); }

  void swap(Slist &other) noexcept { std::swap(header_, other.header_); }
 ...
 private:
  ...
  {
    for (auto header = header_; header; header = header->next) {
...
SLIST_TEMPLATE
inline SLIST_CLASS::~Slist() noexcept { Clear(); }

SLIST_TEMPLATE
SLIST_CLASS::Slist(Slist const &other)
  : slist::NodeAlloctor<T, A>()
  , A()
  , header_(nullptr)
{
  if (other.empty()) {
  ...
  for (; header2 != nullptr; header2 = header2->next, header = header->next) {
