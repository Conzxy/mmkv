...
class AvlTreeBase : protected avl::AvlNodeAllocator<V, Alloc> 
                  , protected Compare 
                  , protected GK {
 ...
 public:
  ...
  void swap(AvlTreeBase& o) noexcept {
   ...
   *   false -- failure
   */
  bool InsertWithDuplicate(value_type const& value, value_type** dup) { return _InsertWithDuplicate(value, dup); }
  bool InsertWithDuplicate(value_type&& value, value_type** dup) { return _InsertWithDuplicate(std::move(value), dup); }
  ...
  V const* Find(K const& key) const {
    return const_cast<AvlTreeBase*>(this)->Find(key);
  }
    ...
    for (; node; node = TO_NODE(_GetNextNode(node))) {
      if (node->left) {
        if (TO_COMPARE(TO_GK(TO_NODE(node->left)->value), TO_GK(node->value)) >= 0) {
          std::cout << "violate(left < mid): " << TO_GK(TO_NODE(node->left)->value) << " >= " << TO_GK(node->value) << "\n";
          ret = false;
        }
      }
      ...
      if (node->right) {
        if (TO_COMPARE(TO_GK(TO_NODE(node->right)->value), TO_GK(node->value)) <= 0) {
          std::cout << "violate(right > mid): " << TO_GK(TO_NODE(node->right)->value) << " <= " << TO_GK(node->value) << "\n";
          ret = false;
        }
      }
 ...
 private:
  void DecreaseCount() noexcept {
    static_cast<D*>(this)->_DecreaseCount();
  }

  void IncreaseCount() noexcept {
    static_cast<D*>(this)->_IncreaseCount();
  }

  void SetCount(size_type n) noexcept {
    static_cast<D*>(this)->_SetCount(n);
  }
