...
class LibcAllocatorWithRealloc {
 public:
  ...
  template <typename U>
  LibcAllocatorWithRealloc(LibcAllocatorWithRealloc<U> const &) noexcept
  {
  }
  ...
  T *reallocate(pointer p, size_t old_n, size_type n) noexcept
  {
    return reinterpret_cast<T *>(
        util::Realloc(p, old_n * sizeof(T), n * sizeof(T)));
  }
...
template <typename T>
inline bool operator==(LibcAllocatorWithRealloc<T> x,
                       LibcAllocatorWithRealloc<T> y)
{
  (void)x;
  (void)y;
  return true;
...
#endif // _MMKV_ALGO_LIBC_ALLOCATOR_WITH_REALLOC_H_
